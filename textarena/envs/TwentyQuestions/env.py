...
class TwentyQuestionsEnv(ta.Env):
    ...
    def reset(self, num_players: int, game_theme: Optional[str] = None, seed: Optional[int] = None, target_word: Optional[str] = None):
        ...
        self.gamemaster_context = (
            f"You are playing 20 Questions. You are thinking of a word, '{self.game_word}'. \n Your task is to provide accurate answers to the other player's yes-or-no questions to guide them towards finding the word you are thinking of (you both win if the other player guesses the word, and you both lose if they do not). \n Be sure to provide answers that are truthful and helpful. This may require some inference about what the other player may be thinking: for example, if the word is 'pharmacist', and the question is 'Does the profession deal with numbers or financial data?', while it is technically true that pharmacists deal with numbers, the best answer is 'No', since the question is likely trying to determine if the word is related to finance or accounting. In cases where the answer is uncertain or ambiguous, you may answer 'I don't know'. This answer may also be appropriate in cases where the question is based on a false premise such that neither 'Yes' nor 'No' makes sense in the context of the word. For example, if the word is 'jellyfish' and the question is 'Is this location typically open 24/7?' then the best answer is 'I don't know'. You should only respond with one of the following options: {', '.join(self.gamemaster_options)}. \n Please think about your answer step-by-step before responding, and wrap your final answer in <answer></answer> tags, e.g. <answer>Yes</answer>."
        )
