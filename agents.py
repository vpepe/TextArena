...
class LLMAgent(ta.agents.OpenRouterAgent):
    ...
    def __call__(self, game_history: list) -> str:
        ...
        if player_questions > 0:
            last_answer = game_history[-1]['message'] if game_history[-1]['player'] == -1 else None
            last_answer = last_answer.lower()
            print("Last answer:", last_answer)
        else:
            last_answer = None

        remaining_questions = max(0, 8 - player_questions)

        print(remaining_questions, player_questions)
        decision = self.decision(formatted_history, remaining_questions, last_answer)
    ...
    def decision(self, history: str, remaining_questions: int, last_answer, max_retries: int = 10) -> str:
...
class QBayesAgent(LLMAgent):
    def __init__(self, openrouter_agent: ta.agents.OpenRouterAgent, k: int = 10):
        ...
        self.k = k
        self.constraints = []
        self.most_recent_constraint = None
        self.eigs = []

    def decision(self, history: str, remaining_questions: int, last_answer, max_retries: int = 10) -> str:
        """Ask if the agent wants to ask more questions or try to guess"""
        if last_answer and self.most_recent_constraint:
            #print("History",history)
            #print("Updating constraints based on last answer")
            #print(last_answer, self.most_recent_constraint)
            new_constraints = {character: result == last_answer for character, result in self.most_recent_constraint.items()}
    ...
    def calculate_eig_for_question(self, candidate_constraint: dict) -> float:
        ...
        for constraint in self.constraints:
            for char_name, constraint in constraint.items():
                if not constraint:
    ...
    def question(self, history, remaining_questions=40, max_retries: int = 5) -> str:
        for attempt in range(max_retries):
            ...
            if question_eigs:
                ...
                self.most_recent_constraint = best_question_info['constraints']
...
class MBayesAgent(LLMAgent):
    def __init__(self, openrouter_agent: ta.agents.OpenRouterAgent, k: int = 1):
        super().__init__(openrouter_agent=openrouter_agent)
        self.openrouter_agent = openrouter_agent
        self.sampling_agent = ta.agents.OpenRouterAgent(model_name="openai/gpt-5")  # Dedicated GPT-5 for sampling
        self.k = k
    ...
    def move(self, history: str, max_retries: int = 10) -> str:
        ...
        # Find character with highest weight
        max_weight_char = max(character_weights.items(), key=lambda x: x[1])[0]
                
        # Format and return the answer
        return f"[{max_weight_char}]"
